...
func renewTokens(c echo.Context, refreshToken *token.RefreshToken) {
	// renew access
	...
	return func(c echo.Context) error {
		...
		if err != nil && c.Request().Method != http.MethodOptions {
			...
			if selectAuth != "" {
				...
				refreshToken, err := tokenRepo.FindRefreshToken(controller.DB, selector)
				if err != nil {
					return next(c)
				}
				...
				if refreshToken.Valid(authenticator) {
					...
					_, err3 := tokenRepo.UpdateRefreshToken(controller.DB, refreshToken)
				...
				if refreshToken.ExpiresOn.Before(time.Now()) {
					tokenRepo.DeleteRefreshToken(controller.DB, refreshToken)
				}
			}
		}
...
func (controller *AuthController) HandleLogin(c echo.Context) error {
	...
	loginUser, err := userRepo.FindUserByEmail(controller.DB, loginRequest.Email)
	switch {
	...
	default:
		if bcrypt.CompareHashAndPassword([]byte(loginUser.PasswordHash), []byte(loginRequest.Password)) == nil {
			...
			if loginRequest.Remember {
				...
				_, err3 := tokenRepo.InsertRefreshToken(controller.DB, refreshToken)
...
func (controller *AuthController) HandleLogout(c echo.Context) error {
	...
	if selectAuth != "" {
		...
		tokenRepo.DeleteRefreshTokenBySelector(controller.DB, sa[0])
	}
