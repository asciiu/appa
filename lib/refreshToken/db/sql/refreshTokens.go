...
func FindRefreshToken(db *sql.DB, selector string) (*refresh.RefreshToken, error) {
	var t refresh.RefreshToken
	err := db.QueryRow(`SELECT 
		...
		token_hash, 
		expires_on 
		FROM refresh_tokens WHERE selector = $1`, selector).
		Scan(&t.ID,
			&t.UserID,
			&t.Selector,
			&t.TokenHash,
...
func InsertRefreshToken(db *sql.DB, token *refresh.RefreshToken) (*refresh.RefreshToken, error) {
	sqlStatement := `insert into refresh_tokens (
		id, 
		user_id, 
		selector, 
		token_hash, 
		expires_on) values ($1, $2, $3, $4, $5)`
...
func DeleteRefreshToken(db *sql.DB, token *refresh.RefreshToken) (*refresh.RefreshToken, error) {
...
func UpdateRefreshToken(db *sql.DB, token *refresh.RefreshToken) (*refresh.RefreshToken, error) {
